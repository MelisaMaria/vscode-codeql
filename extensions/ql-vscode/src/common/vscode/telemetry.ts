...
async function notifyTelemetryChange() {
  ...
  do {
    chosenItem = await window.showInformationMessage(
      "The CodeQL extension by GitHub will now use your VS Code telemetry settings instead of its own. You currently have VS Code telemetry enabled, but disabled for the CodeQL extension. CodeQL extension telemetry will now be enabled. You can learn more about telemetry by clicking the links below.",
      { modal: true },
      continueItem,
      vsCodeTelemetryItem,
      codeqlTelemetryItem,
